    ...
    try {
      ...
      out.write("    ");
            ...
            try {
                ...
                CallableStatement cs = con.prepareCall("{call addbooks(?,?,?,?,?,?)}");

                cs.setString("bname", name);
                cs.setString("authorn", au);
                cs.setInt("quan", q);
                cs.setString("dept", d);
                cs.registerOutParameter("ex", java.sql.Types.VARCHAR);
                cs.registerOutParameter("ex2", java.sql.Types.VARCHAR);

                cs.execute();
                String ex = cs.getString("ex2");
                ...
                else if(e.equals("Book Added Successfully")) {
                out.println("<script>");
                out.println("alert('" + e +"');");
